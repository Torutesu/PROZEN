...
const inputFile = process.argv[2] ?? "examples/bet-spec.example.json";
const absolutePath = path.resolve(process.cwd(), inputFile);
const schemaPath = path.resolve(process.cwd(), "schemas/bet-spec.schema.json");
...
const run = async () => {
  ...
  if (ok) {
    process.stdout.write(`Bet Spec is valid: ${absolutePath}\n`);
    return;
  }

  process.stderr.write(`Bet Spec is invalid: ${absolutePath}\n`);
